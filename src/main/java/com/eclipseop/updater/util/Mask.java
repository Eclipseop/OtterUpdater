...
public class Mask extends BigInteger {
	...
	public static final Mask INVOKEDYNAMIC = of(Opcodes.INVOKEDYNAMIC);
	...
	public int operand = -1, var = -1, increment = -1;
	...
	public static List<AbstractInsnNode> prev(AbstractInsnNode ain, Mask... masks) {
		List<AbstractInsnNode> insns = new ArrayList<>();
		for (Mask mask : masks) {
			AbstractInsnNode prev = prev(ain, mask.distance, mask);
			if (prev == null)
				return null;
			ain = prev;
			insns.add(prev);
	...
	public static List<List<AbstractInsnNode>> findAll(final MethodNode mn, final Mask... masks) {
		final List<List<AbstractInsnNode>> all = new ArrayList<>();
		for (final AbstractInsnNode ain : mn.instructions.toArray()) {
			final List<AbstractInsnNode> result = next(ain, masks);
			if (result != null)
				all.add(result);
		}
	...
	public static AbstractInsnNode first(final MethodNode mn, final Mask... masks) {
		final AbstractInsnNode first = mn.instructions.getFirst();
		for (final Mask mask : masks) {
			return first != null ? mask.matches(first) ? first : next(first, masks).get(0) : null;
		}
		return null;
	}
	...
	private Mask from(final String bigInt, final int distance,
					  final String owner, final String desc, final int operand,
					  final int var, final Object cst, final int increment,
					  final boolean descContains) {
		...
		mask.var = var;
		mask.cst = cst;
		mask.increment = increment;
		mask.descContains = descContains;
		return mask;
	}

	private Mask from(final int distance, final String owner,
					  final String desc, final int operand, final int var,
					  final Object cst, final int increment, final boolean descContains) {
		return from(toString(), distance, owner, desc, operand, var, cst, increment, descContains);
	}

	public Mask distance(int distance) {
		return from(distance, owner, desc, operand, var, cst, increment, descContains);
	}
	...
	public Mask var(final int var) {
		return from(distance, owner, desc, operand, var, cst, increment, descContains);
	}

	public Mask cst(final Object cst) {
		return from(distance, owner, desc, operand, var, cst, increment, descContains);
	}

	public Mask increment(final int increment) {
		return from(distance, owner, desc, operand, var, cst, increment, descContains);
	}

	@Override
	public final Mask or(BigInteger bi) {
		return from(super.or(bi).toString(), distance, owner, desc, operand, var, cst, increment, descContains);
	}

	@Override
	public final Mask xor(BigInteger bi) {
		return from(super.xor(bi).toString(), distance, owner, desc, operand, var, cst, increment, descContains);
	}

	@Override
	public final Mask and(BigInteger bi) {
		return from(super.and(bi).toString(), distance, owner, desc, operand, var, cst, increment, descContains);
	}

	@Override
	public final Mask andNot(BigInteger bi) {
		return from(super.andNot(bi).toString(), distance, owner, desc, operand, var, cst, increment, descContains);
	}

	@Override
	public final Mask not() {
		return from(super.not().toString(), distance, owner, desc, operand, var, cst, increment, descContains);
	}

	public final boolean matches(AbstractInsnNode ain) {
		if (of(ain.opcode()).and(this).bitCount() == 0)
			return false;
		if (ain instanceof FieldInsnNode) {
			FieldInsnNode fin = (FieldInsnNode) ain;
			return (owner == null || fin.owner.equals(owner))
					&& (desc == null || (descContains ? fin.desc.contains(desc)
					: fin.desc.equals(desc)));
		} else if (ain instanceof MethodInsnNode) {
			MethodInsnNode min = (MethodInsnNode) ain;
			return (owner == null || min.owner.equals(owner))
					&& (desc == null || (descContains ? min.desc.contains(desc)
			...
			return var == -1 || operand == -1 || vin.var == var;
		} else if (ain instanceof IincInsnNode) {
			final IincInsnNode iinc = (IincInsnNode) ain;
			return increment == -1 || operand == -1 || iinc.incr == increment;
		} else if (ain instanceof LdcInsnNode) {
			final LdcInsnNode ldc = (LdcInsnNode) ain;
			return ldc.cst == null || operand == -1 || ldc.cst.equals(cst);
		}
		return true;
	}
}